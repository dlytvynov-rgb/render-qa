...
По КОЖНІЙ правці з усіх джерел звір ПІСЛЯ проти ДО і визнач ОДИН з: застосована (yes) / не застосована (no) / стало гірше (regressed) / НЕ можна підтвердити з рендера (needs_human). Не оцінюй нічого поза цими правками.

ВАЖЛИВО про needs_human — став його ЛИШЕ коли сама ПРИРОДА правки невимірна з кадру: точний розмір (см/дюйми/відсотки, «трохи ширше/товще»), тонкий фініш (матовий vs глянець), грубість/глибина затирки (grout), масштаб текстури тканини, «трохи тепліший колір».
АЛЕ якщо правка про ДИСКРЕТНУ зміну об'єкта (додати / прибрати / замінити / переробити / інший колір чи матеріал) і ти БАЧИШ, що об'єкт справді змінено, додано чи прибрано — став yes (правку АДРЕСОВАНО), НАВІТЬ якщо не впевнений, чи ТОЧНО збігається зі стилем/еталоном. «Замінити на стиль з фото», «переробити», «зменшити повторюваність», «гармонізувати з …» — якщо об'єкт явно інший/перероблений/доданий/прибраний → це yes, а НЕ needs_human.
Запам'ятай: needs_human ≠ «важко звірити з референсом», needs_human = «фізично неможливо ПОБАЧИТИ з цього кадру». Не ховайся в needs_human від складних, але ВИДИМИХ правок.

ВІДПОВІДАЙ ТІЛЬКИ ОДНИМ JSON-обʼєктом:
{"id":"${check.id}","status":"ok"|"warn"|"fail","note":"стисло по кожній правці — що зроблено/не зроблено","zone":{"x":0,"y":0,"w":0,"h":0},"changes":[{"text":"текст правки","done":"yes"|"no"|"regressed"|"needs_human","zone":{"x":0,"y":0,"w":0,"h":0},"conf":0}]}
...
Тобі дано (у такому порядку):
Зображення 1 = ПОВНИЙ кадр ДО (контекст усієї сцени).
Зображення 2 = ПОВНИЙ кадр ПІСЛЯ (контекст усієї сцени).
Зображення 3 = ЗБІЛЬШЕНИЙ кроп зони ДО (деталь).
Зображення 4 = ЗБІЛЬШЕНИЙ кроп зони ПІСЛЯ (деталь).${hasVisual ? "\nДалі — повний ВІЗУАЛЬНИЙ ТУ-ДУ (референс-специфікація клієнта: анотації + картинки-еталони). Знайди в ньому запис/еталон, що стосується САМЕ цієї правки, і звір ПІСЛЯ з ним — матеріал, колір, форма, кількість. Саме еталон визначає, що означає «зроблено»." : ""}

ВАЖЛИВО: для правок про КІЛЬКІСТЬ / РОЗТАШУВАННЯ / всю сцену («додати більше/менше», «додати ще N», «зменшити кількість», «перехід через кадр», «переставити») дивись на ПОВНІ кадри (1-2) — на кропі цього не порахувати. Для дрібних деталей (матеріал, шов/затирка, колір, форма, метал) — на КРОПИ (3-4).
...
ДОКАЗИ-FIRST: спершу подумки опиши, що РЕАЛЬНО видно на кропі ПІСЛЯ (об'єкт, матеріал, колір, форма), і ЛИШЕ тоді визнач вердикт. Флагуй «не зроблено» (no) ТІЛЬКИ якщо на кропі видно, що правку НЕ застосовано. Якщо об'єкт правки ВИДИМО змінено / додано / прибрано / перероблено — став yes (правку АДРЕСОВАНО), навіть якщо не впевнений у точному збігу зі стилем еталону. Став "done":"needs_human" ЛИШЕ коли ПРИРОДА правки невимірна з кадру (точний розмір у см/дюймах, матовість/глянець, грубість/глибина затирки, масштаб текстури, «трохи тепліший колір») — а НЕ просто «важко звірити з референсом».

ВІДПОВІДАЙ ТІЛЬКИ ОДНИМ JSON-обʼєктом:
{"done":"yes"|"no"|"regressed"|"needs_human","conf":0,"note":"що бачиш на кропі + вердикт, одне-два речення"}`;
}
